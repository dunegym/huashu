...
```
=== TM-30 颜色质量评估 ===
保真度指数 (Rf): 91.79091728161299
色域指数 (Rg): 106.0794926996035
颜色保真度评价: 优秀
色域评价: 理想色域
```
...
```
计算的睡眠指标已保存至: results/sleep_metrics_calculated.xlsx
统计分析摘要已保存至: results/statistical_analysis_summary.xlsx
箱形图已保存至: results/boxplots_comparison.png
小提琴图已保存至: results/violinplots_comparison.png
柱状图已保存至: results/barplots_comparison.png
相关性热力图已保存至: results/correlation_heatmaps.png
雷达图比较已保存至: results/radar_comparison.png
密度分布图已保存至: results/density_distributions.png
...
生成的图表包括：
- 箱形图分析（boxplots_comparison.png）
- 小提琴图分析（violinplots_comparison.png）
- 柱状图分析（barplots_comparison.png）
- 指标相关性热力图（correlation_heatmaps.png）
- 各环境睡眠指标雷达图比较（radar_comparison.png）
- 概率密度分布图（density_distributions.png）
...
事后检验结果 (Bonferroni校正):
  助眠灯 vs 普通LED: p=2.7522 [无显著差异]
  助眠灯 vs 黑暗: p=0.0136 [显著差异，助眠灯差于黑暗环境]
  普通LED vs 黑暗: p=0.1010 [无显著差异]
...
================================================================================
统计说明:
- 正态性检验: Shapiro-Wilk检验 (p>0.05为正态)
- 正态数据: 重复测量方差分析(RM-ANOVA) + 配对t检验
- 非正态数据: Friedman检验 + Wilcoxon符号秩检验
- 多重比较校正: Bonferroni方法 (p值×3)
- 显著性水平: α=0.05
================================================================================
```